...
export default function AboutPage() {
  ...
  return (
    <PageShell>
      <PageHero eyebrow="About Us" title="Crafting structures that last a lifetime" subtitle="Integrated engineering & construction with safety, precision, and integrity at the core." breadcrumb={[{ label: "About Us" }]} />

      <section className="container-x py-24">
        <div className="grid items-center gap-12 md:grid-cols-2">
          <div className="relative">
            <img src={aboutImg} alt="AEPL engineers on site" loading="lazy" className="w-full rounded-3xl object-cover aspect-[4/5]" />
          ...
          <div>
            ...
            <p className="mt-6 text-lg text-muted-foreground">At AEPL, we deliver durable engineering and construction solutions by combining advanced materials, robust design, modern technology, and a strong commitment to quality, safety, and sustainability.</p>
            ...
            <Link to="/contact" className="mt-10 inline-flex items-center gap-3 rounded-full bg-primary px-7 py-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition">
              Talk to our team →
            </Link>
          </div>
        </div>
      ...
      <section className="bg-dark text-dark-foreground py-20 rounded-[2.5rem] mx-2 md:mx-6 mt-16">
        <div className="container-x text-center">
          ...
          <h2 className="font-display text-4xl font-bold md:text-5xl max-w-3xl mx-auto">Building the future on a foundation of excellence</h2>
          <p className="mt-6 max-w-2xl mx-auto text-dark-foreground/70 text-lg">We combine proven engineering methodologies with a relentless commitment to safety, enabling our clients to bring even the most complex industrial projects to fruition on schedule and on budget.</p>
        </div>
      </section>
      ...
      <section className="container-x pb-24">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-muted-foreground max-w-xl mx-auto">Our leadership team brings decades of combined experience in engineering, construction, and project management.</p>
        </div>
          ...
          {teamMembers.map(member => (
            <div key={member.name + member.role} className="group text-center">
              <div className="overflow-hidden rounded-2xl aspect-[3/4]">
                <img src={member.img} alt={member.name} loading="lazy" className="w-full h-full object-cover transition duration-500 group-hover:scale-105" />
              </div>
              <div className="mt-4">
        ...
        <div className="mx-auto max-w-3xl space-y-3">
          {faqs.map((f, i) => (
            <div key={f.q} className="rounded-2xl border bg-card overflow-hidden">
              <button onClick={() => setOpenIdx(openIdx === i ? null : i)} className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left" aria-expanded={openIdx === i}>
